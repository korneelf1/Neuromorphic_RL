...
def main():
    ...
    plt.savefig('SNN_in128x2out_50e3_20hz.png') # save the figure with name SNN_246_200Hz_100e3
    
    # Save the data of the x and y data of the plot in a txt file with the same naming of the figure
    with open('SNN_in128x2out_50e3_20hz.txt', 'w') as f:
        for i in range(len(global_model.episode_times)):
            f.write(f'{i}\t{global_model.episode_times[i]}\n')
    
    
    global_model.save_model(path='A3C/past_trainings/Figures/SNN_in128x2out_50e3_20hz_lower_entropy_loss.pt')
    global_model.join()
