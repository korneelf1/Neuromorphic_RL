...
class ActorCriticSNN_LIF_Small(torch.nn.Module):
    ...
    def forward(self, inputs, nr_passes = 1):
        ...
        return val, actions
class ActorCriticSNN_LIF_withbuffer(torch.nn.Module):
    def __init__(self, num_inputs, action_space, hidden1=64,hidden2=64, inp_min = torch.tensor([0]), inp_max=  torch.tensor([2.5]),bias=False, nr_passes = 1, device = 'cpu'):

        super(ActorCriticSNN_LIF_withbuffer, self).__init__()
        ...
        self.buffer = snn.Synaptic(beta=.8,alpha=.7, learn_alpha=True, learn_beta=True, reset_mechanism="none") # make none so that it can adapt for batch size
       # membranes at t = 0
        self.mem1     = self.lif1.init_leaky()
        self.mem2     = self.lif2.init_leaky()
        ...
        self.mem_buf, self.mem_cur = self.buffer.init_synaptic()
        self.inp_min = inp_min
        self.inp_max = inp_max
    ...
    def forward(self, batch):
        ...
        if len(self.mem_buf)==0:
            self.mem_buf = torch.zeros((batch.shape[0], self.hidden2), device=self.device)
        values = []
            
        inputs = torch.cat((inputs, self.mem_buf), dim = 1)
        
        # inputs = torch.tensor(inputs).to(torch.float32)
        inputs[:,0:self.num_inputs] = (inputs[:,0:self.num_inputs] - self.inp_min)/(self.inp_max - self.inp_min)
        ...
        # buffer
        _, self.mem_buf,self.cur_buf = self.buffer(self.lin_buffer(spk2), self.mem_buf, self.mem_cur)
