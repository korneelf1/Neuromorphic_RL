...
class ReplayBuffer:
    ...
    def sample(self, batch_size, sequence_length):
        ...
        for experience in batch:
            ...
            start_index = random.randint(0, len(state) - sequence_length)

            states.append(state[start_index:start_index+sequence_length])
            actions.append(action)
            rewards.append(reward)
            next_states.append(next_state[start_index:start_index+sequence_length])
            dones.append(done)
...
class Buffer_PPO():
    ...
    def __init__(self, config:dict, observation_space:spaces.Box, action_space_shape:tuple, device:torch.device) -> None:
        ...
        hidden_state_size = config["spiking"]["hidden_state_size"]
        self.neuron_type = config["spiking"]["neuron_type"]
        ...
        self.mem_pot = torch.zeros((self.n_workers, self.worker_steps, hidden_state_size))
    ...
    def prepare_batch_dict(self) -> None:
        ...
        # Add collected recurrent cell states to the dictionary
        samples["mem_pot"] =  self.mem_pot
        if self.neuron_type == "synaptic":
            samples["mem_cur"] = self.mem_cur
        ...
        self.samples_flat = {}
        for key, value in samples.items():
            if not key == "mem_pot" and not key == "mem_cur":
